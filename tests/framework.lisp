    ...
    (if (not (plist-p param))
        ...
        (progn
          ...
          (let ((pname (getf param :name))
                ...
                (p-nut (getf param :nullable-underlying-type))
                ...
                (p-def-kind (getf param :default-kind))
                (p-def-type (getf param :default-type))
            ...
            (when (and p-nut (not (stringp p-nut)))
              (setf valid nil)
              (utils:format-red *error-output* "[SCHEMA ERROR] ~A: :nullable-underlying-type must be a string, got ~S~%" context p-nut))
            ...
            (when (not (eq (if p-has-def t nil) (if p-def-kind t nil)))
              (setf valid nil)
              (utils:format-red *error-output* "[SCHEMA ERROR] ~A: :has-default is ~S but :default-kind presence is ~S~%" context p-has-def p-def-kind))
            (when (not (eq (if p-has-def t nil) (if p-def-type t nil)))
              (setf valid nil)
              (utils:format-red *error-output* "[SCHEMA ERROR] ~A: :has-default is ~S but :default-type presence is ~S~%" context p-has-def p-def-type))
            (when (and p-def-kind (not (member p-def-kind '(:null :bool :number :char :string :enum :unrepresentable))))
              (setf valid nil)
              (utils:format-red *error-output* "[SCHEMA ERROR] ~A: :default-kind must be one of :null/:bool/:number/:char/:string/:enum/:unrepresentable, got ~S~%" context p-def-kind))
            (when (and p-def-type (not (stringp p-def-type)))
              (setf valid nil)
              (utils:format-red *error-output* "[SCHEMA ERROR] ~A: :default-type must be a string, got ~S~%" context p-def-type)))))
    valid))
    ...
    (if (not (plist-p method))
        ...
        (progn
          ...
          (let ((mname (getf method :name))
                ...
                (m-nut-ret (getf method :nullable-underlying-return-type))
            ...
            (when (and m-nut-ret (not (stringp m-nut-ret)))
              (setf valid nil)
              (utils:format-red *error-output* "[SCHEMA ERROR] ~A: :nullable-underlying-return-type must be a string, got ~S~%" context m-nut-ret))
    ...
    (if (not (plist-p prop))
        ...
        (progn
          ...
          (let ((pname (getf prop :name))
                ...
                (p-nut (getf prop :nullable-underlying-type))
    ...
    (if (not (plist-p event))
        ...
        (progn
          ...
          (let ((ename (getf event :name))
                ...
                (e-nut (getf event :nullable-underlying-type))
            ...
            (when (and e-nut (not (stringp e-nut)))
              (setf valid nil)
              (utils:format-red *error-output* "[SCHEMA ERROR] ~A: :nullable-underlying-type must be a string, got ~S~%" context e-nut))
    ...
    (if (not (plist-p field))
        ...
        (progn
          ...
          (let ((fname (getf field :name))
                ...
                (f-nut (getf field :nullable-underlying-type))
            ...
            (when (and f-nut (not (stringp f-nut)))
              (setf valid nil)
              (utils:format-red *error-output* "[SCHEMA ERROR] ~A: :nullable-underlying-type must be a string, got ~S~%" context f-nut))
